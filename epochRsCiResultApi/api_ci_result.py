...
def post_tekton_task(workspace_id):
    ...
    try:
        # image tagの生成
        if re.match("^refs/tags/", request.json['git_branch']):
...
if __name__ == "__main__":
    app.run(debug=eval(os.environ.get('API_DEBUG', "False")), host='0.0.0.0', port=int(os.environ.get('API_CI_RESULT_PORT', '8000')), threaded=True)